...
Tile::Tile():m_cost(0.0f)
{
	...
	//setTileCost(0.0f);
	//can be used instead of assignment in constructor
...
{
	return m_neighbourTiles[position];

}

void Tile::setNeighbourTile(NeighbourTile position, Tile* tile)
{
	m_neighbourTiles[position] = tile;
}
...
void Tile::setTileCost(float cost)
{
	...
	// format to 1 deciaml place
	std::stringstream stream;
	stream << std::fixed << std::setprecision(1) << cost;	// manipulates cost with precision saves to stream
	const std::string cost_string = stream.str();	// converts stream to string
